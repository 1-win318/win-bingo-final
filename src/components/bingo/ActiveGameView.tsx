...
export function ActiveGameView({ onGameEnd, selectedIds, cartels, player, otherPlayers }: ActiveGameViewProps) {
  ...
  // ... (Game logic effects are unchanged) ...
  useEffect(() => {
  ...
  if (winInfo && winningCard) {
    return (
      <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center animate-in fade-in duration-500 p-4">
        <div className="w-full max-w-sm bg-gradient-to-b from-gray-800 to-gray-900 rounded-2xl border-2 border-yellow-500 shadow-2xl p-4 pt-6 text-center">
          ...
          <div className="my-4">
            ...
            <BingoCard 
              ...
              markedNumbers={new Set(winInfo.winningLine)} // Highlights the winning line
              isMini={false}
            />
          </div>

          <p className="text-xs text-gray-400 animate-pulse">Returning to lobby...</p>
        </div>
      </div>
    );
  }
