...
export function CartelSelection({ cartels, onBack, onPlay, selectedIds, setSelectedIds, timer, balance }: CartelSelectionProps) {
  ...
  const displayedCartels = cartels.slice(0, 400); // Ensure we only show up to 400 tickets
  ...
  return (
    <div className="min-h-screen bg-[#0b0e1a] text-white flex flex-col font-body items-center">
      ...
      <div className="w-full max-w-md grid grid-cols-4 text-center bg-[#14182d] flex-none">
         <div className="py-2 border-r border-black/20">
          <p className="text-[8px] text-white/40 font-bold uppercase tracking-wider">Main Wallet</p>
          <p className="font-bold text-sm">{balance.toFixed(0)}</p>
        </div>
        <div className="py-2 border-r border-black/20">
          <p className="text-[8px] text-white/40 font-bold uppercase tracking-wider">Play Wallet</p>
          <p className="font-bold text-sm">0</p>
        </div>
        <div className="py-2 border-r border-black/20">
          <p className="text-[8px] text-white/40 font-bold uppercase tracking-wider">Stake</p>
          <p className="font-bold text-sm">{stake}</p>
        </div>
        <div className="py-2 relative">
          <p className="text-[8px] text-white/40 font-bold uppercase tracking-wider">Timer</p>
          <p className="font-bold text-lg text-red-500">{timer}s</p>
        </div>
      </div>
      ...
      {/* Selected Cards Preview */}
       <div className="w-full max-w-md flex-none p-2 bg-black/30">
        <p className="text-[10px] text-center text-white/50 mb-2 uppercase font-bold tracking-wider">Selected Cards ({selectedIds.length}/4)</p>
        {selectedIds.length > 0 ? (
            <div className="flex justify-center gap-2 overflow-x-auto scrollbar-hide py-1">
                {selectedIds.map(id => {
                    ...
                    return (
                    <div key={id} className="w-[100px] min-w-[100px] flex-none">
                        <BingoCard data={cartel.board} />
                    </div>
                    );
                })}
            </div>
